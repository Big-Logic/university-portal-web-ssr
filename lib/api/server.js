...
/**
 * The 401 body for an access token that didn't verify.
 *
 * Exported because serverRequest isn't the only thing that needs it: a
 * Route Handler wanting the caller's *identity* rather than a proxied
 * call (app/api/users/me/avatar/signature) has to verify the same
 * cookie and must answer with the same codes. lib/api/client.js
 * branches on `error.code` to decide whether a refresh could possibly
 * help, so a second spelling of these would quietly break the retry.
 *
 * Note this can't be replaced by getCurrentUser() in a Route Handler:
 * that reads headers Proxy sets, and `/api/*` is not in proxy.js's
 * matcher, so those headers are never present here.
 */
export function authFailureBody(tokenStatus) {
  return { error: TOKEN_FAILURE[tokenStatus] ?? INVALID_TOKEN_FAILURE };
}
...
export async function serverRequest(path, { method = "GET", body } = {}) {
  ...
  if (tokenStatus !== TOKEN_VALID) {
    return { data: authFailureBody(tokenStatus), status: 401 };
  }
