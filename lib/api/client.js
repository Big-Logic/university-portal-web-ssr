...
export async function clientRequest(
  ...
  if (!res.ok) {
    // The code rides along with the message so callers can branch on
    // *why* a request failed. The message can't carry that -- it's
    // prose written for the user and free to be reworded. /reset-password
    // is the first caller that needs it: INVALID_RESET_TOKEN is the
    // difference between "try that again" and "this link is spent,
    // go get another one".
