...
// them) and proxy.js (which reads them). Centralized here so
...
// JWT_REFRESH_EXPIRES_IN_DAYS (see the API repo's .env) -- a small,
// real coupling between the two apps worth knowing about if either
// value changes on the API side. Defined here, next to the cookie
// options they're always passed alongside, so the login/refresh Route
// Handlers and lib/api/server.js can't drift out of sync with each other.
export const ACCESS_MAX_AGE = 15 * 60; // 15 minutes, in seconds
export const REFRESH_MAX_AGE = 30 * 24 * 60 * 60; // 30 days, in seconds
