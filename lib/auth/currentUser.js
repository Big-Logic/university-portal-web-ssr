 ...
 * signature via jose (./jwt.js) and forwarded `sub`/`role` as
 * request headers, so this just reads those back -- they're
 * cryptographically trustworthy by the time they get here.
 *
 * Lives here rather than in lib/api deliberately: everything in there
 * is a real HTTP call to the Express API, whereas this never leaves
 * the process -- it's a request-header read on data ./jwt.js already
 * vouched for. Same reason it needs neither API_URL nor the
 * access-token cookie.
 *
 ...
 * `apiRequest("/api/v1/users/me")` from lib/api/server.js instead,
 * since that data was never in the token to begin with.
 */
